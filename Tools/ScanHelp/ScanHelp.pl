...
print "-=Eric MacDonald's HELP Scan=-\n\n";
...
$portsub1=<STDIN>;
chop($portsub2);
print "Operation 1=Only URLs, 2=Only Server Title, Rest=both y/n: ";
$ogURLs=<STDIN>;
chop($ogURLs);
...
sub processHelp {
print "Connecting $_[0]:$_[1] $PortDesc{$_[1]}\n";
$fullvagabond="";
$restGoFor=0;
$sock=IO::Socket::INET->new("$_[0]:$_[1]");
if(!($sock)){
close($sock);
...
while($sockbuffer=<$sock>){
$command=substr($sockbuffer,0,index($sockbuffer," "));
$command=lc($command);
$params=substr($sockbuffer,index($sockbuffer," ")+1);
chop($params);
if($params=~/\r/){
chop($params);
}
...
if($ogURLs == 2){
print "$params is for $_[0]:$_[1]\n";
print FILE "$params is for $_[0]:$_[1]\n";
close($sock)
}else{
print "\n\n<<<This server is $params ($_[0]:$_[1] (".$PortDesc{$_[1]}."))>>>\n";
print FILE "\n\n<<<This server is $params ($_[0]:$_[1] (".$PortDesc{$_[1]}."))>>>\n";
...
if($sockbuffer=~/Bad Request/){
close($sock)
}
if($ogURLs != 2){
if($sockbuffer eq "\n" && !($restGoFor)){
if($ogURLs == 1){
print FILE "The commands that this server allows is as follows:\n";
print "The commands that this server allows is as follows:\n";
}
$restGoFor=1;
}

if($restGoFor){
$fullvagabond.=$sockbuffer;

if($command ne "commands"){
if($ogURLs == 1){
print $sockbuffer;
print FILE $sockbuffer;
...
if(length($fullvagabond) eq $len || length($fullvagabond) > $len){
close($sock);
}
}
}
